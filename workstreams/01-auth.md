...
- Sign-up always creates an `EMPLOYEE`. The role is never a form field and never read from the request. The only way to become an `ADMIN` is `/register/organization`, which creates a brand new organisation and its founding administrator in one transaction; it can never attach to an organisation that already exists.
- Revised 2026-09-22: each organisation now has its own `joinCode` (`Organization.joinCode`, unique, generated on creation). `/register/employee` looks the organisation up by that code instead of a single global `ORG_JOIN_CODE` env var pinned to one hard-coded demo organisation, so sign-up now supports real multi-tenant use instead of one hackathon org. Founders see their organisation's code on `/account`. The demo one-click login buttons and `DEMO_LOGIN` are removed; `/api/health` (`HEALTH_CHECK` env var) no longer depends on the named demo accounts.
...
- admin and employee: every database query needs a `where: { orgId }` (or a join through `User.orgId`). This now matters more than before: `/register/employee` join codes are per-organisation, so multiple real organisations can exist, each with real members. `SessionUser` carries `orgId` as of 2026-09-22 (`web/src/lib/auth/session.ts`) — until the repositories filter on it, an administrator of one organisation would see another organisation's expenses, findings and cases.

## Progress log

_Newest first. Each entry: date, what changed, what is next, blockers._

- 2026-09-22: At the user's request, moved sign-up off the hackathon single-organisation model now that the team is building the real product. `Organization` gained a `joinCode` field (unique, nullable so the existing seeded demo org needs no backfill); `/register/organization` generates one per new organisation (`generateJoinCode` in `web/src/lib/auth/org.ts`, retried on the ~1-in-1.7e12 collision) and `/register/employee` looks an organisation up by its own code instead of matching the global `ORG_JOIN_CODE` env var against the single hard-coded `org_auditx_demo`. Founders can now find and copy their organisation's join code on `/account` (new `Organization` panel, `join-code.tsx`), which they could not see anywhere before this change. `SessionUser` gained `orgId` to make that lookup possible. Removed the "Continue as the demo administrator/employee" one-click sign-in (`demo.ts`, `demo-access.tsx`, `demoLogin`, `DEMO_LOGIN`) since a real product should not ship a no-password login for two fixed accounts; `/api/health` (kept, renamed its toggle to `HEALTH_CHECK`) no longer checks those two named accounts, only env vars and generic database connectivity. Explicitly out of scope, per the user: the shared Supabase database itself is untouched — `org_auditx_demo`, its two named logins (`admin@auditx.local`, `employee@auditx.local`) and the 45-employee synthetic dataset under it all still exist, just with no join code, so nobody can self-register into that organisation any more. Updated `register.test.ts`, `register-create.test.ts`, `.env.example`, `DEPLOY.md`, `README.md`, `web/README.md` to match; `npx prisma generate` was run (codegen only, no `db push`). Next: `npm run db:push` against the shared database is still needed before this works live — the `joinCode` column does not exist there yet — and after that, `admin`/`employee` should filter every query on `orgId` (see Needs from others) since multiple real organisations can now hold real data side by side. Not committed or pushed.
...
- 2026-09-20: Added one-click demo sign-in on the login page: "Try the demo" with "Continue as the demo administrator" and "Continue as the demo employee". `demoLogin` in `web/src/app/(auth)/login/actions.ts` reads only a role name and signs in with the seeded accounts held in `web/src/lib/auth/demo.ts`, so no password reaches the browser. On unless `DEMO_LOGIN=off`; documented in DEPLOY.md, README.md and .env.example. Five tests added (`demo-actions.test.ts`); all 200 web tests pass, `tsc` and eslint are clean, and both buttons were clicked in a browser to reach `/admin` and `/employee`. Next: none. Note: the constants in `demo.ts` must match `prisma/seed.ts`. No commit or push.
