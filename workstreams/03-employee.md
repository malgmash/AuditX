...
- 2026-09-20: Restored the employee sidebar profile block (initials, name, email, department, role) and soft brand-tint particle dots behind the shell (hidden under reduced motion), keeping the remote topbar with NotificationBell and Account. Next: finish section 7 live hold clear.
- 2026-09-20: Polished the `/employee` overview UI within DESIGN.md. One score band (number + sparkline), quieter empty holds, labeled why/next on held items, bordered expenses table and status panel, clearer page header and loading skeleton.
