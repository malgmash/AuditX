...
- 2026-09-20: Month-end timestamps in the score history now end in .999 instead of .999999, because Postgres keeps milliseconds and rounded the last microsecond into the next month, which put the wrong month on the chart. Scores were regenerated with recompute. Three live decisions were made through the admin screens on the real data: one reversal, one decline, one accept. A decision or reversal takes about 10 seconds in development, most of it the full rescore; a scoped rescore for one person would be the next speed-up.
